...
@PageTitle("Source charts")
@Route("source-charts")
public class SourceCharts extends VerticalLayout {
    ...
    public SourceCharts() {
        ...
        layout.add(bufferCapacityField, devicesCountField, aField, bField, lambdaField);
        add(layout);
    ...
    private void getSourceCharts() {
        ...
        for (int i = startValue; i < endValue; i += stepValue) {
            Main.start(i, bufferCapacity, devicesCount, MainView.requestCountForCharts, false);
    ...
    private void getFirstChart(List<DataSeriesItem> refuseProb) {
        ...
        conf.setTitle(new Title("Dependency of refuse probability and sources count"));
    ...
    private void getSecondChart(List<DataSeriesItem> timeInSystem) {
        Chart chart = new Chart(ChartType.AREA);
        Configuration conf = chart.getConfiguration();
        conf.setTitle(new Title("Dependency of all time in system and sources count"));
    ...
    private void getThirdChart(List<DataSeriesItem> usageCoefficient) {
        Chart chart = new Chart(ChartType.AREA);
        Configuration conf = chart.getConfiguration();
        conf.setTitle(new Title("Dependency of devices usage coefficient and sources count"));
